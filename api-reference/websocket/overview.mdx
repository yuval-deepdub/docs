...
| Format | Description |
|--------|-------------|
| `mp3` | Compressed audio, smallest size. **Default.** |
| `wav` | Raw PCM audio bytes **without WAV headers** (headerless). |
| `opus` | High-quality compressed audio, efficient for streaming. |
| `mulaw` | 8-bit µ-law encoding, commonly used in telephony. |

<Warning>
  Unlike the REST API, the WebSocket `wav` format returns **headerless raw PCM bytes** (no WAV file header). You must prepend a WAV header yourself if saving to a `.wav` file, or use the data directly for real-time playback.
</Warning>
